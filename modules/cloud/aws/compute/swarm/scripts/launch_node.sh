...
# Get a meta-data value of an EC2 instance given the API token and attribute name
...
# Join the current EC2 as a node to the docker swarm given its swarm token
join_swarm() {
  local TOKEN=$1
...
# If there is no swarm token and the current instance is the first in the list
if [ "$SWARM_TOKEN" == "NONE" ] \
